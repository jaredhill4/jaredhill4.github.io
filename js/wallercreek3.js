	...
	$(window).load(function() {
		...
		$("#and_the_winner_is").delay(2750).fadeIn(500);
		$("#scroll_down").delay(3000).fadeIn(500).animate({bottom:100}, 500);
	});
...
$(document).ready(function()
{
	...
	$('#winner').waypoint(function(event, direction) {
		
		if (direction === 'down') {
			$('#nav').addClass('fixed');
			$('#nav').removeClass('absolute');
			$('#nav_winner').addClass('active');
			...
			$("#and_the_winner_is").delay(0).fadeOut(0);
			...
			$("#and_the_winner_is").delay(1250).fadeIn(500);
			$("#scroll_down").delay(1500).fadeIn(500).animate({bottom:100}, 500);
		}
	}, {
		offset: 70
	});
	...
	$("#finalists_slider_team2").click(
	  function () {
		...
		$('#next_team').fadeOut(500);
		$('#prev_team').fadeOut(500);
		$('#design_team1_details').fadeOut(500);
		$('#design_team3_details').fadeOut(500);
		$('#design_team4_details').fadeOut(500);
		//$('#finalists_subhead').fadeOut(500);
		$.scrollTo('#winner', 500, {offset:-70}, 'easeInOutExpo');
	...
	$(window).resize(function() {
		...
		$('#intro').css("height", height);  // Eliminate the heights value
	});
	...
	$(".winner_board_inner").draggable({
		axis: "x",
		containment: [-2370, 0, 0, 0]
	});
